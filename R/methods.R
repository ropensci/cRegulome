...
#' @param ob A \link{cmicroRNA} or \link{cTF} object such as this returned by calling \link{cmicroRNA} or \link{cTF}.
...
print.cTF <- function(ob, ...) {
  p <- paste('A cTF object: transcription factor expression correlations in Cancer\n',
             'Contains:\n',
             length(ob$studies), 'Cancer study/ies:', paste(head(ob$studies), collapse = ' '), '\n',
             length(ob$tf), 'Transcription factor/s:', paste(head(ob$TF), collapse = ' '), '\n',
             length(ob$features), 'features:', paste(head(ob$features), collapse = ' '), '\n')
  cat(p)
}
...
#'
#' A dot plot of microRNA/TF correlation in a single study of TCGA. When the object \link{cmicroRNA}/\link{cTF}
#' contains more than one TCGA studies, the argument \code{study} is a requirment.
#'
#' @param ob A \link{cmicroRNA} or \link{cTF} object such as this returned by calling \link{cmicroRNA} or \link{cTF}.
...
#'  min_cor = .5)
#'
#' # convert to cmicroRNA object and back
#' ob <- cmicroRNA(dat)
#' tidy(ob)
#' }
#' @import dplyr tidyr
...
#' @inheritParams plot
#'
#' @return A tidy \code{data.frame} of four columns. \code{tf} is the official symbol of the gene containin the transcription
#'         factor, \code{feature} is the features/genes, \code{cor} is the corresponding expression correaltions and \code{study}
#'          is TCGA study ID.
#' @examples
#' \dontrun{
#' # get data for 2 transcription factors in the ACC study
...
#'
#' Count and plot the numbers of microRNA correlated features in \code{cmicroRNA} object.
#'
#' @inheritParams plot
#' @return A venn diagram with a ciccle or an ellipses for each microRNA and the number of correlated features.
#' @examples
#' \dontrun{
#' # get data for 2 microRNAs in the ACC study
#' dat <- get_mir(c('hsa-let-7b', 'hsa-mir-134'),
...
#'
#' Count and plot the numbers of transcription factors correlated features in \code{cTF} object.
#'
#' @inheritParams plot
#' @return A venn diagram with a ciccle or an ellipses for each transcription factor and the number of correlated features.
#' @examples
#' \dontrun{
#' # get data for 2 transcription factors in the ACC study
#' dat <- get_tf(c('AFF4', 'ESR1'),
...
#'
#' \code{\link[UpSetR]{upset}} of sets of microRNAs and their correlated features in a TCGA study.
#'
#' @inheritParams plot
...
#'
#' \code{\link[UpSetR]{upset}} of sets of transcription factors and their correlated features in a TCGA study.
#'
#' @inheritParams plot
#' @return An \code{\link[UpSetR]{upset}} plot
#' @examples
