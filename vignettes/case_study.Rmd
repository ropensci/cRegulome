...
Next, we truned to the Cistrome Cancer databse to get the TF-gene correlation data. Cistrom Cancer is based on an integrative analysis of The Cancer Genome Atlas (TCGA) and public ChIP-seq data. The data comes in text files, one for each TF, each file contains the calculated pearson's correlation between the TF ana all target genes in each of the TCGA studies.  
...
V(p)$size <- ifelse(V(p)$type == 'gene', 2, log(degree(p)))
