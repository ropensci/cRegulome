...
miRCancer databases. Cistrome Cancer is based on an integrative analysis of
The Cancer Genome Atlas (TCGA) and public ChIP-seq data. It provides
calculated correlations of (n=) transcription factors and their target
genes in (n=) cancer study. In addition, cistrome cancer provides the
transcription factors regulatory potential to target and non-target genes.
miRCancer uses TCGA data and TargetScan annotations to correlate known
microRNAs (n =) and target and non-target genes in (n = ) cancer studies.  

## Database file  
cRegulome obtains a pre-build sqlite database file of the cistrome cancer
and miRCancer databases. The details of this build is provided at
(cRegulomedb) in addition to the scripts used to pull, formate and deposit
...
to genes mappings.  Two indecies are created to facilitate the database
...
Here we show an example of such a query. Then, we illusterat how this qeury
excuted on the database using basic `RSQLite` and `dbplyr` which is what
...
dat_mir2 <- tbl(conn, 'cor_mir') %>%
    select(mirna_base, feature, ACC, BLCA) %>%
...
the reglome element, features for the gene hits, studies for the TCGA
